<template>
  ...
  <el-sub-menu :index="selfPath" v-else>
    ...
    <MenuItem v-for="(item, index) in childrenRoutes" :key="index" :parentPath="selfPath" :data="item"/>
  </el-sub-menu>
</template>
...
export default defineComponent({
  ...
  setup(props) {
    ...
    const selfHidden = computed(() => {
      return  (props.data.meta?.hidden == true)
    })
    ...
    const showOne = computed(() => {
      ...
      let res = (!childrenRoutes.value || childrenRoutes.value.length == 0 || childrenRoutes.value.length == 1)
      return res
    })
